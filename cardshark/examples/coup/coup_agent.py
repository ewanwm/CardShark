...
class CoupHumanAgent(HumanAgent):
    ...
    def _get_block_challenge_mask(self):
        ...
        for value in range(3):
            if self._game._get_mask(
                np.array([0, n_targets - 1, value, 0]), self._player_id
            ):
                mask[value] = 1
    ...
    def _get_action_mask(self):
        ...
        for action in range(n_actions):
            ...
            for player_id in range(self._game.n_players):
                if self._game._get_mask(
                    np.array([action, player_id, 0, 0, 0]), self._player_id
                ):
                    mask[action] = 1
    ...
    def _get_target_mask(self, action: int):
        ...
        for target in range(n_targets):
            if self._game._get_mask(
                np.array([action, target, 0, 0, 0]), self._player_id
            ):
                mask[target] = 1.0
    ...
    def _get_action(self):
        ...
        if self._game._game_state == coup_engine.ActionState:
            ...
            ret[0] = self._get_user_input(
                self._game._action_space.action_max[0], self._get_action_mask()
            )
            ...
            if np.sum(self._get_target_mask(ret[0])[:-1]) >= 1:
                ...
                ret[1] = self._get_user_input(
                    self._game._action_space.action_max[1],
                    self._get_target_mask(ret[0]),
                )
            else:
                ret[1] = self._game.n_players - 1

        elif self._game._game_state in [
            coup_engine.BlockingGeneralState,
            coup_engine.ChallengeGeneralState,
            coup_engine.BlockOrChallengeState,
        ]:
