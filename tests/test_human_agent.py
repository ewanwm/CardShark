...
from examples.coup import coup_engine, coup_player, coup_agent

nPlayers = 4

## Build the environments for training and evaluation
train_logger = log.Logger(name = "Train_env_Logger", logLevel = log.logLevels.kInfo, toFile = False)
...
eval_logger = log.Logger(name = "Eval_env_Logger", logLevel = log.logLevels.kInfo, toFile = False)
...
def runMatchSingleHuman(human_agent, robot_agents, collect=False, random=False, train=True):
...
def runMatchMultiAgent(agents, train: bool, collect: bool = False, random: bool = False):
    """ run a match and train agent """
    ## TODO: add random collection policy to the MultiAgent that can be used to randomly sample environment
...
human_agent = coup_agent.CoupHumanAgent(name = coup_agent.MAGENTA + "Kyle" + coup_agent.RESET)
...
for i in range(nPlayers - 1):
    ...
    train_py_env.player_list[1 + i].name = bot_colours[i] + "bot_" + str(i) + coup_agent.RESET
